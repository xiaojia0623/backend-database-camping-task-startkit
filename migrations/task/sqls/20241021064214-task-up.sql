...
SELECT 
  ...
  email AS 信箱,
  role AS 角色,
  created_at AS 建立日期時間,
  updated_at AS 更新日期時間
FROM "USER"
LIMIT 3;
...
INSERT INTO "COACH_LINK_SKILL" (coach_id, skill_id)
VALUES (
  (SELECT id FROM "COACH" WHERE user_id = (SELECT id FROM "USER" 
...
(SELECT id FROM "SKILL" WHERE name = '重訓'),'重訓基礎課','2024-11-25 14:00:00','2024-11-25 16:00:00',10, 'https://test-meeting.test.io');
...
((SELECT id FROM "USER" WHERE email = 'richman@hexschooltest.io'),
 (SELECT id FROM "COURSE" WHERE user_id = (SELECT id FROM "USER" 
    WHERE email = 'lee2000@hexschooltest.io')),'2024-11-24 16:00:00','即將授課');
...
SELECT *
FROM "COURSE_BOOKING" WHERE user_id = (
  SELECT id FROM "USER" WHERE email = 'wXlTq@hexschooltest.io');
...
SELECT "SKILL".name AS 專長名稱 , count("COACH".id) AS coach_total
FROM "SKILL"
